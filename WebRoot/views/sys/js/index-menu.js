	...
	$("#requestFrom").validate({
		   ...
		   submitHandler: function(form) {
				...
				if($("#id").val()!=""){
					...
					params["id"]=$("#id").val();
					$.ajax({
				        url:getContextPath()+"/api/sys/menu/update", 
				        type: "POST",
				        dataType:"json",
				        data: params, 
					...
					$.ajax({
				        url:getContextPath()+"/api/sys/menu/insert", 
				        type: "POST",
				        dataType:"json",
				        data: params, 
				        success: function(resultBean, status, xhRequest){
...
//加载树
function loadMenuTree(f){
	$("#parentId").empty();
	$.ajax({
        url:getContextPath()+"/api/sys/menu/loadTreeMenus", 
        type: "POST",
        dataType:"json",
        data: "", 
        success: function(data, status, xhRequest){
        	topLoaded();
       	 	var t = $("#min-ztree");
       	 	treeData=data.data;
            var at = $.fn.zTree.init(t, setting, data.data);
            ...
            //展开父节点
            if(f){
            	var node = at.getNodeByParam("id", $("#nowparentId").val(), null);
            	if(node){
            	    at.expandNode(node);
            	}
            }
...
function findMenu(){
	$("#id").val(clickNode.id);
	$("#nowparentId").val(clickNode.parentId);
	$("#name").val(clickNode.name);
	$("#orderNo").val(clickNode.orderNo);
	$("#status").val(clickNode.status);
	$("#uri").val(clickNode.uri);
	$("#imgsrc").attr("src",clickNode.imgPath?getFsDomain()+clickNode.imgPath:'../../resources/images/logo/pic1.jpg').removeAttr("style");
...
function deleteOldImg(oldImagePath){
	if(oldImagePath){
  		 $.post(getFsDomain() + "/api/delete","cutPhoto=false"+oldImagePath,function(resultBean){
...
/*function deleteMenu(){
	...
	_confirmDialog("亲，你确定要删除吗？","删除温馨提示",function(e){
		if(e){
			topLoading();
			$.ajax({
		        url:getContextPath()+"/api/sys/menu/delete", 
		        type: "POST",
		        dataType:"json",
...
function success(resultBean){
	//兼容IE
   	...
   	if(obj.success){
  		//显示图片
  		$("#imgsrc").attr("src", getFsDomain() + obj.data.path);
